...
export function choose(message){
 ...
 }).map((item,index)=>({item,score:(item.needs.includes(needs[0])?10:0)+(audience&&item.populations.includes(audience)?5:0)+(item.availability.state==="accepting"?1:0)-index/1000})).sort((a,b)=>b.score-a.score).slice(0,3).map(x=>x.item)
}
export function understanding(message,prior={}){const namedArea=message.match(/\b(?:downtown(?: san francisco)?|tenderloin|soma|south of market|mission(?: district)?|civic center|bayview|haight|castro|sunset|richmond|san francisco)\b/i)?.[0];const located=message.match(/(?:near|at|around|corner of)\s+([^,.]+)/i)?.[1];const location=located||namedArea;const audience=/woman|female/i.test(message)?"adult woman":/man|male/i.test(message)?"adult man":/family|children|child|kids/i.test(message)?"family with children":/\badult\b/i.test(message)?"adult":prior.audience;return {...prior,need:/food|meal|hungry/i.test(message)?"food":/drug|recovery|substance/i.test(message)?"recovery support":/shelter|bed|safe/i.test(message)?"shelter or safety":prior.need,location:location||prior.location,timing:/tonight|now|urgent|right away/i.test(message)?"needed now":prior.timing,audience}}
